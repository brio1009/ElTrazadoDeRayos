...
private:
  ...
  {
    return (i == 0 ? (head != nullptr ? ((head->fnv_1a_hash(head->text_size - 1) ^ text_ptr[0]) * fnv_prime) : fnv_offset) : ((fnv_1a_hash(i - 1) ^ text_ptr[i]) * fnv_prime));
  }
  ...
  { // NOLINT
    if (head != nullptr)
      head->copy_to_recurse(beg, end);
    std::size_t sz_to_cpy = (end - beg < static_cast<int>(text_size) ? end - beg : text_size);
    std::memcpy(beg, text_ptr, sz_to_cpy);
    beg += sz_to_cpy;
  }
  constexpr literal_str_list(const char *aStr, unsigned int N,
                             const literal_str_list *aHead = nullptr) : text_ptr(aStr), text_size(N), head(aHead) {}

public:
  template <unsigned int N>
  constexpr literal_str_list(const char (&aStr)[N], // NOLINT
                             const literal_str_list *aHead = nullptr) : text_ptr(aStr), text_size(N - 1), head(aHead)
  {
